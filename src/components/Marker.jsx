...
export default function Marker({ map, point, color }) {
  useEffect(() => {
    const icon = L.icon({
      iconUrl: require(`../media/marker-icon-${color}${
        point.isOvernight ? "-night" : ""
      }.png`),
      iconSize: point.isOvernight ? [39, 47] : [25, 41], // size of the icon
      iconAnchor: point.isOvernight ? [19, 47] : [12, 41], // point of the icon which will correspond to marker's location
    ...
    if (map) {
      ...
      marker.bindPopup(
        ...
        Link: <a href="${
          point.link
        }" target="_blank" rel="noopener noreferrer">${point.link}</a> <br>
